...
index fa67859ca6d1d14f668ed0bb2ae14a6a00048ed1..80c3d4563fdd2fd36b548ef38570e7f10ed7a9e4 100644
--- a/bot.py
+++ b/bot.py
@@ -1,3771 +1,3941 @@
-import json
...
+    conn = get_db()
+    c = conn.cursor()
+    c.execute("SELECT COUNT(*), SUM(CASE WHEN status='accepted' THEN 1 ELSE 0 END), SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END), SUM(reward) FROM webapp_leads WHERE owner_telegram_id = ?", (owner_id,))
...
+        user = _parse_webapp_user(payload.get('initData', ''))
+        upsert_webapp_user(user)
+        user['is_admin'] = is_admin(user['telegram_id'])
+        user['stats'] = get_webapp_stats(user['telegram_id'])
+        return jsonify(user)
+
+    @app.get('/api/leads')
+    def api_leads():
+        user = _parse_webapp_user(request.args.get('initData', ''))
+        upsert_webapp_user(user)
+        conn = get_db()
+        c = conn.cursor()
+
+        if is_admin(user['telegram_id']):
+            c.execute('''SELECT l.id, l.owner_telegram_id, l.fio, l.city, l.status, l.orders, l.reward, l.created_at, u.avatar
+                         FROM webapp_leads l LEFT JOIN webapp_users u ON l.owner_telegram_id=u.telegram_id
+                         ORDER BY l.id DESC''')
+        else:
+            c.execute('''SELECT l.id, l.owner_telegram_id, l.fio, l.city, l.status, l.orders, l.reward, l.created_at, u.avatar
+                         FROM webapp_leads l LEFT JOIN webapp_users u ON l.owner_telegram_id=u.telegram_id
+                         WHERE l.owner_telegram_id = ? ORDER BY l.id DESC''', (user['telegram_id'],))
+
+        leads = []
+        for row in c.fetchall():
+            leads.append({'id': row[0], 'owner_telegram_id': row[1], 'fio': row[2], 'city': row[3], 'status': row[4], 'orders': row[5], 'reward': row[6], 'created_at': row[7], 'owner_avatar': row[8]})
...
+        parts = raw.split()
+        if len(parts) < 3:
+            return jsonify({'error': 'Используйте формат: Фамилия Имя Город'}), 400
+        fio = " ".join(parts[:2])
+        city = " ".join(parts[2:])
+
+        conn = get_db()
...
+    app = build_webapp()
+    thread = threading.Thread(target=lambda: app.run(host=WEBAPP_HOST, port=WEBAPP_PORT, debug=False, use_reloader=False), daemon=True)
+    thread.start()
